...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA6_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org6.example.com/ca && ls *_sk)
    export BYFN_CA7_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org7.example.com/ca && ls *_sk)
    export BYFN_CA8_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org8.example.com/ca && ls *_sk)
  fi
  ...
  # now run the end to end script for channel1
  docker exec cli scripts/script1.sh $CHANNELONE_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $NO_CHAINCODE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi

  # now run the end to end script for channel2
  docker exec cli scripts/script2.sh $CHANNELTWO_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $NO_CHAINCODE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Test failed"
    exit 1
  fi
...
function generateCerts() {
  ...
  echo
  echo "Generate CCP files for Org1 - Org8"
  ./ccp-generate.sh
}
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "### Generating channel1 configuration transaction 'channel1.tx' ###"
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelOne -outputCreateChannelTx ./channel-artifacts/channel1.tx -channelID $CHANNELONE_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelOne -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNELONE_NAME -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelOne -outputAnchorPeersUpdate \
    ./channel-artifacts/Org2MSPanchors.tx -channelID $CHANNELONE_NAME -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile ChannelOne -outputAnchorPeersUpdate \
    ./channel-artifacts/Org3MSPanchors.tx -channelID $CHANNELONE_NAME -asOrg Org3MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  set -x
  configtxgen -profile ChannelOne -outputAnchorPeersUpdate \
    ./channel-artifacts/Org4MSPanchors.tx -channelID $CHANNELONE_NAME -asOrg Org4MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelTwo -outputCreateChannelTx ./channel-artifacts/channel2.tx -channelID $CHANNELTWO_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelTwo -outputAnchorPeersUpdate ./channel-artifacts/Org5MSPanchors.tx -channelID $CHANNELTWO_NAME -asOrg Org5MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelTwo -outputAnchorPeersUpdate ./channel-artifacts/Org6MSPanchors.tx -channelID $CHANNELTWO_NAME -asOrg Org6MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelTwo -outputAnchorPeersUpdate ./channel-artifacts/Org7MSPanchors.tx -channelID $CHANNELTWO_NAME -asOrg Org7MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile ChannelTwo -outputAnchorPeersUpdate ./channel-artifacts/Org8MSPanchors.tx -channelID $CHANNELTWO_NAME -asOrg Org8MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
CHANNELONE_NAME="mychannel1"
CHANNELTWO_NAME="mychannel2"
